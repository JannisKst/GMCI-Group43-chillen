    ...
    app.innerHTML = `
        ...
        <div style="position: absolute; top: 20px; left: 20px;">
            ...
            <div id="groupPopup" style="display: none; position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background-color: white; padding: 20px; border-radius: 8px; box-shadow: 0 0 15px rgba(0,0,0,0.2); width: 300px;">
                ...
                <input type="text" id="newGroupName" placeholder="Enter group name" style="width: 100%; padding: 10px; font-size: 16px; margin-bottom: 15px;">
                <button onclick="createGroup()" style="padding: 10px 20px; background-color: rgb(79, 153, 223); color: white; border: none; cursor: pointer; font-size: 18px; border-radius: 5px;">Create Group</button>
                <button onclick="closePopup()" style="padding: 10px 20px; background-color: red; color: white; border: none; cursor: pointer; font-size: 18px; border-radius: 5px;">Cancel</button>
            </div>
        </div>
    `;
}
...
function goBack(courseName, prof) {
    loadCourseSelection();
}
